...
func (c *ddsClient) Collect(namespace string, ch chan<- prometheus.Metric) error {
	if c.desc == nil {
		c.desc = newDescOfInstnaceInfo(namespace, "acs_mongodb", []string{"regionId", "instanceId", "dbType", "desc", "status"})
	}
	...
	go func() {
		...
		for hasNextPage, pageNum := true, 1; hasNextPage != false; pageNum++ {
			...
			totalCount += len(response.DBInstances.DBInstance)
...
func init() {
	register("acs_mongodb", func(s1, s2, s3 string, l log.Logger) (Collector, error) {
